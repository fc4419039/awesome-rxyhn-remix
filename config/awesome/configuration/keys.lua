...
local scratchpad = require("configuration.scratchpad")
        ...
        awful.key({modkey, ctrl}, "t", function()
            ...
            if night_mode then
                ...
                naughty.notify({
                    ...
                    timeout = 2, fg = "#ffffff", bg = "#8B4513"
                })
            end
        end,
        {description = "toggle night mode", group = "hotkeys"}),
})
...
-- Screenshots
    awful.key({}, "Insert", function()
        awful.spawn.with_shell("date +%T >> /tmp/insert-debug.log; mkdir -p ~/Pictures && maim ~/Pictures/Captura_$(date +%F_%T).png")
        awful.spawn.with_shell(os.getenv("HOME") .. "/.config/awesome/scripts/notif-play.sh")
        naughty.notify({ title = "Captura", text = "Pantalla completa guardada", ignore_effect = true })
    end,
    {description = "take a full screenshot", group = "hotkeys"}),

    awful.key({alt}, "Insert", function()
        awful.spawn.easy_async_with_shell("maim -s | xclip -selection clipboard -t image/png", function()
            awful.spawn.with_shell(os.getenv("HOME") .. "/.config/awesome/scripts/notif-play.sh")
            naughty.notify({ title = "Captura", text = "Área copiada al portapapeles", ignore_effect = true })
        end)
    end,
...
awful.mouse.append_global_mousebindings({
    ...
    awful.button({}, 1, function()
        ...
        if stats_tooltip and stats_tooltip.visible then
            stats_tooltip_hide()
        end
    end),
