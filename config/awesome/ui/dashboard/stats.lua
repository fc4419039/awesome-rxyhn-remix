...
awesome.connect_signal("signal::volume", function(value, muted)
    if vol and vol.slider then
        local fill_color = muted and beautiful.xcolor8 or vol_color
        local vol_value = value or 0

        vol.slider.value = vol_value
        vol.slider.color = fill_color
        if vol_tooltip then vol_tooltip.markup = helpers.colorize_text(vol_value .. "%", vol_color) end
    end
end)

awesome.connect_signal("signal::brightness", function (value)
    if brightness and brightness.slider then
        brightness.slider.value = value or 0
        if brightness_tooltip then brightness_tooltip.markup = helpers.colorize_text((value or 0) .. "%", brightness_color) end
    end
end)

awesome.connect_signal("signal::cpu", function(value)
    if cpu and cpu.slider then
        cpu.slider.value = value or 0
        if cpu_tooltip then cpu_tooltip.markup = helpers.colorize_text((value or 0) .. "%", cpu_color) end
    end
end)

awesome.connect_signal("signal::ram", function(used, total)
    if ram and ram.slider and total and total > 0 then
        local r_average = (used / total) * 100
        local r_used = string.format("%.1f", used / 1000) .. "G"

        ram.slider.value = r_average
        if ram_tooltip then ram_tooltip.markup = helpers.colorize_text(r_used, ram_color) end
    end
end)
